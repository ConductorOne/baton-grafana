...
| :--- | :--- | :--- | :--- |
| `is_externally_synced` | Boolean | When Grafana returns it (see below) | `true` when the user's organization role is managed by an external identity provider; `false` when the role is managed locally in Grafana. |
...
`is_externally_synced` is Grafana's native `isExternallySynced` flag, meaning the user's organization role is managed by an external identity provider (such as Entra ID or grafana.com SSO). It is surfaced only when Grafana returns the flag, which depends on the connector mode:

* **Grafana Cloud:** the org-users endpoint returns the flag, so the attribute is always present and mirrors Grafana's value exactly. Note that in Cloud every user authenticates through grafana.com (so `auth_labels` is effectively always `grafana.com`); a user whose organization role is managed locally still reports `false`.
* **Self-hosted Grafana:** the global user list endpoint does not return the flag, so the connector cannot determine role sync and omits the attribute entirely. Use `auth_labels` to see how these users authenticated.
