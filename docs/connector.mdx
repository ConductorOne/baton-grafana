...
Team membership can be granted and revoked (`/api/teams/{id}/members`). Grafana RBAC roles that a team holds are synced and can be assigned to or removed from a team (`/api/access-control/teams/{id}/roles`); the connector syncs the IRM and OnCall plugin roles (for example **Schedules Editor**), which require Grafana Cloud or Enterprise. Service accounts are synced with their organization role; they are read-only.

<Note>
**Roles are optional**

IRM / OnCall RBAC roles are available only on Grafana Cloud and Enterprise. Role sync is disabled by default — enable the **Role** resource type in the connector's settings in C1 when your instance has access-control. Teams and service accounts sync by default.
</Note>
...
The connector always syncs teams and service accounts (in addition to orgs and users). Opting in to the RBAC **role** resource type also requires access-control read/write for IRM/OnCall role assignments. Use an **Admin** service-account role (or equivalent permissions covering `teams:read`, `serviceaccounts:read`, and when opted in `roles:read` / team role assignment). A narrower token that previously synced only users/orgs may fail List after this upgrade.
