...
Use an **Admin** service-account role (or equivalent permissions covering `teams:read`, `serviceaccounts:read`, and `roles:read`). A narrower token that previously synced only users/orgs may fail List after this upgrade.
