...
Use an **Admin** service-account role (or equivalent permissions covering `teams:read`, `serviceaccounts:read`, and when Role sync is enabled `roles:read`). A narrower token that previously synced only users/orgs may fail List after this upgrade.
