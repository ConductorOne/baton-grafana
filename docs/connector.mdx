...
These six actions cover **sync only**. When provisioning is enabled (`BATON_PROVISIONING`), granting and revoking access calls separate write endpoints that need additional RBAC actions:

| Permission | Endpoint | Needed for |
| :--- | :--- | :--- |
| `org.users:add` | `POST /api/org/invites` | Inviting/adding a user to the org |
| `org.users:write` | `PATCH /api/org/users/{id}` | Updating a user's org role |
...
Beyond team and role permissions, self-hosted org and user sync call instance-admin-scoped endpoints:
...
These three endpoints are part of Grafana's legacy admin API and are gated by the account's **Grafana Admin** (server admin) flag rather than a granular RBAC action — an equivalent admin-level permission is required for each.
