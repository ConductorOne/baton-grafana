...
Use an **Admin** service-account role (or equivalent permissions covering `teams:read`, `serviceaccounts:read`, and when Role sync is enabled `roles:read` plus `teams.roles:read`). A narrower token that previously synced only users/orgs may fail List after this upgrade.
...
When Role sync is enabled (Grafana Enterprise), that account also needs `roles:read` plus `teams.roles:read`. On OSS, leave Role sync disabled — the access-control API is not present.
