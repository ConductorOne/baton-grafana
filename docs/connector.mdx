...
Before listing anything, the connector validates the credential with `GET /api/org`. That endpoint is gated by a Grafana **basic role**, not by an RBAC action: a service account set to **No basic role** returns `403` there even when every RBAC action below is granted, and assigning `orgs:read` does not satisfy it. Setting the basic role to **Viewer** does.
...
| `teams.roles:read` | `GET /api/access-control/teams/{id}/roles` | The RBAC roles each team holds |
...
Team sync reads each team's members and RBAC roles, so `teams.permissions:read` and `teams.roles:read` are required even with Role sync turned off; only `roles:read` is specific to Role sync. A narrower token that previously synced only users/orgs may fail List after this upgrade.

<Note>
**`roles:read` and `teams.roles:read` cannot be separated with Grafana's built-in roles**

If you are building a least-privilege credential with Role sync disabled, you may want `teams.roles:read` without `roles:read`. No built-in Grafana role expresses that split: `fixed:roles:reader` and `fixed:roles:writer` each grant both actions together, and the only basic roles that grant `roles:read` (`basic:admin` and `basic:grafana_admin`) also grant `teams.roles:read`. `basic:viewer` and `basic:editor` grant neither. Isolating `teams.roles:read` requires a **custom role**.
</Note>
...
On Grafana Enterprise that account also needs `teams:read` (`GET /api/teams/search`), `teams.permissions:read` (team membership, from `GET /api/teams/{id}/members`), and `teams.roles:read`, which team sync reads for every team, plus `roles:read` when Role sync is enabled. Self-hosted reads users from the global users endpoint rather than `GET /api/org/users`, so `org.users:read` — required in Cloud mode — does not apply here. See the permission table in the **Grafana Cloud** tab above for the full endpoint-by-endpoint mapping, and note that `teams.roles:read` cannot be granted without `roles:read` using Grafana's built-in roles — that split requires a custom role. On OSS, leave Role sync disabled — the access-control API is not present, and the connector skips that step instead of failing.
