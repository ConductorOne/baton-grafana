...
* **Grafana Cloud:** the value mirrors Grafana's native `isExternallySynced` flag verbatim. It reflects **only** whether the user's organization role is managed by an external identity provider (role sync) — it is **not** set merely because the user signed in through an external provider. In Grafana Cloud every user authenticates through grafana.com, so `auth_labels` is effectively always `grafana.com`; a user whose organization role is managed locally therefore reports `false` even though their `auth_labels` show `grafana.com`.
