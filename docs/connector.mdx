...
**Account provisioning** (`CreateAccount`, invite-based in Cloud mode):

| Permission | Endpoint | Needed for |
| :--- | :--- | :--- |
| `org.users:add` | `POST /api/org/invites` | Inviting a brand-new user into the org |

**Entitlement provisioning** (Grant/Revoke on the Organizations and Teams resource types):

| Permission | Endpoint | Needed for |
| :--- | :--- | :--- |
| `org.users:write` | `PATCH /api/org/users/{id}` | Updating an existing member's org role |
| `org.users:remove` | `DELETE /api/org/users/{id}` | Removing a user from the org |
| `teams.permissions:write` | `POST /api/teams/{id}/members` | Adding a user to a team |
| `teams.permissions:write` | `DELETE /api/teams/{id}/members/{userId}` | Removing a user from a team |

A customer who enables entitlement provisioning without account provisioning does not need `org.users:add`: in Cloud mode, Grant only ever updates the role of a user who is already an org member (via `PATCH /api/org/users/{id}`), or fails outright if the user isn't already a member — it never calls the invite endpoint. `POST /api/org/invites` is reached only by `CreateAccount`.
...
When provisioning is enabled (`BATON_PROVISIONING`), self-hosted also calls write endpoints:

| Endpoint | Needed for |
| :--- | :--- |
| `POST /api/admin/users` | Account provisioning — creating a new user (`CreateAccount`) |
| `GET /api/users/{id}` | Account provisioning — looking up an existing user when `CreateAccount` finds the account already exists |
| `DELETE /api/admin/users/{id}` | Account provisioning — deleting a user |
| `GET /api/users/{id}/orgs` | Entitlement provisioning — checking a user's existing org memberships before Grant/Revoke |
| `POST /api/orgs/{id}/users` | Entitlement provisioning — adding a user to an organization with a role (Grant) |
| `DELETE /api/orgs/{id}/users/{userId}` | Entitlement provisioning — removing a user from an organization (Revoke) |

Like the read endpoints above, these write endpoints are part of Grafana's legacy admin API and are gated by the account's **Grafana Admin** (server admin) flag.
