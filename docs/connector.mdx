...
Use an **Admin** service-account role (or equivalent permissions covering `teams:read`, `teams.roles:read`, and `serviceaccounts:read`; Role sync additionally needs `roles:read`). Team sync reads each team's RBAC roles, so `teams.roles:read` is required even with Role sync turned off. A narrower token that previously synced only users/orgs may fail List after this upgrade.
