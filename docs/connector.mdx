...
`CreateAccount`'s already-exists handling looks up the existing account by paging the global users list (`GET /api/users`, already required for user sync above), not by ID — so no additional permission beyond user sync is needed for that path.

**Entitlement provisioning** (Grant/Revoke on organization membership):
